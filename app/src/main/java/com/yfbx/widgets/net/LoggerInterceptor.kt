...
class LoggerInterceptor(private val onPrintLog: ((String) -> Unit)? = null) : Interceptor {
    ...
    private fun logRequest(chain: Interceptor.Chain, request: Request) {
        ...
        if (!hasRequestBody) {
            ...
            if (isPlaintext(buffer)) {
                if (onPrintLog != null) {
                    onPrintLog.invoke(buffer.readString(UTF8))
                } else {
                    printBody(buffer.readString(UTF8))
                }
    ...
    private fun logResponse(chain: Interceptor.Chain, request: Request): Response {
        ...
        if (!HttpHeaders.hasBody(response)) {
            ...
            if (contentLength != 0L) {
                Log.i(TAG, "")
                printBody(buffer.clone().readString(UTF8))
            }
    ...
    private fun printBody(buffer: String) {
        val array = buffer.toCharArray()
        val lineSize = 1000
        val groupSize = array.size / lineSize + 1
        for (i in 0 until groupSize) {
            val start = i * lineSize
            var end = (i + 1) * lineSize
            end = if (end <= array.size) end else array.size
            Log.i(TAG, String(array.copyOfRange(start, end)))
        }
    }
}